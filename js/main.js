;(function () {
	...
	var highlightsAnimate = function() {
		var highlights = $('#fh5co-highlights');
		if ( highlights.length > 0 ) {	

			highlights.waypoint( function( direction ) {
										
				if( direction === 'down' && !$(this.element).hasClass('animated') ) {


					setTimeout(function() {
						highlights.find('.to-animate').each(function( k ) {
							var el = $(this);
					...
					setTimeout(function() {
						highlights.find('.to-animate-2').each(function( k ) {
							var el = $(this);
	...
	var enhancedButtons = function() {
		
		// Hero section buttons
		$('.hero-buttons .btn').on('click', function(e) {
			e.preventDefault();
			var target = $(this).data('nav-section');
			
			if (target) {
				var topVal = ( $(window).width() < 769 ) ? 0 : 58;
				$('html, body').animate({
					scrollTop: $('div[data-section="' + target + '"]').offset().top - topVal
				}, 800);
			}
		});

		// About section buttons
		$('.about-cta .btn').on('click', function(e) {
			e.preventDefault();
			...
			} else if (action === 'Descargar CV') {
				// Simulate CV download
				alert('Descarga de CV iniciada... (Esta es una simulación)');
			}
		});

		// Contact form functionality
		$('#fh5co-contact .form-group input[type="submit"]').on('click', function(e) {
			e.preventDefault();
			...
			if (!name || !email || service === 'Selecciona un servicio' || !message) {
			...
			$('#service').val('Selecciona un servicio');
			$('#date').val('');
		});

		// Smooth scroll for all navigation links
		$('a[data-nav-section]').on('click', function(e) {
			e.preventDefault();
			var section = $(this).data('nav-section');
			var topVal = ( $(window).width() < 769 ) ? 0 : 58;
		...
		// Services section button functionality
		$('#fh5co-menus .btn').on('click', function(e) {
			e.preventDefault();
			var topVal = ( $(window).width() < 769 ) ? 0 : 58;
			$('html, body').animate({
				scrollTop: $('div[data-section="contact"]').offset().top - topVal
